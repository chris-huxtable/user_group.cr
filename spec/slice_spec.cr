...
describe "Slice" do
  it "does accept null terminated pointer" do
    ...
    ptr[0] = slice_0.to_unsafe()
    ptr[1] = slice_1.to_unsafe()
    ptr[2] = slice_2.to_unsafe()
    ptr[3] = slice_3.to_unsafe()
    ptr[4] = Pointer(UInt8).null

    slice = Slice.new(ptr, limit: 8, read_only: true)
    slice.size.should eq(4)
    slice[0].should eq(slice_0.to_unsafe())
    slice[1].should eq(slice_1.to_unsafe())
    slice[2].should eq(slice_2.to_unsafe())
    slice[3].should eq(slice_3.to_unsafe())
